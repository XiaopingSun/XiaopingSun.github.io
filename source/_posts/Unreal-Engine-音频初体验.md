...
为了测试音频空间化的效果，需要在编辑器构造的场景中放置一个音源的载体，类比于现实环境中的音响、收音机一类的可发声装置。将之前创建的 SoundWave 资产拖入到场景中即可自动创建一个 Ambient Sound Actor 并挂载一个 Audio Component。Unreal Engine 采用 ECS 架构，Ambient Sound Actor 是这个发声装置的物理载体，Audio Component 则负责音频相关的计算。
...
submix 子混音类似于 Cubase、Logic 这种音频工作站的 bus，submix 之间可以互相连接，最终输出到主 submix 输出到监听设备。引擎默认会自带几个内置 submix，除主 submix 外，还有主混音 submix，主 eq submix，为了兼容旧版本 Audio Volume 的混响以及 Sound Class Mix 的 eq 调节，可以在项目设置中音频选项双击查看内置 submix：
